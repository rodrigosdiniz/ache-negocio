...
export default function ResultadosComponent() {
  ...
  const query = normalizarTexto(queryRaw)
    ...
    const fetchEmpresas = async () => {
      ...
      if (query.trim()) {
        ...
        if (error) {
          ...
          const resultadosFiltrados = (data || []).filter((empresa) => {
            const nome = normalizarTexto(empresa.nome)
            const categoria = normalizarTexto(empresa.categoria)
            const cidade = normalizarTexto(empresa.cidade)
            return nome.includes(query) || categoria.includes(query) || cidade.includes(query)
          })

          setEmpresas(resultadosFiltrados)
        }
      } else {
        setEmpresas([])
      }
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold mb-6">Resultados para: "{queryRaw}"</h1>
